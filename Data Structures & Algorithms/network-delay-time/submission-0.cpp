    ...
    int networkDelayTime(vector<vector<int>>& times, int n, int k) 
    {
        ...
        while (pq.size()) // djikstra's
        {
            ...
            else
            {
                ...
                std::cout << "At " << node << " curr distance is " << currDistance << std::endl;

                distances[node] = std::min(distances[node], currDistance); // update distance with new shortest
                std::cout << "New distance for " << node << " is " << distances[node] << std::endl;
                ...
                for (auto& edge : times)
                {
                    if (edge[0] == node) // we are at an edge of this curr node
                    {
                        ...
                        std::cout << "Found: " << target << " in " << currDistance + timeWeight << "s" << std::endl;
                    } 
                } 
            }
        }
