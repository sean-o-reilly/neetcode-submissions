    ...
    vector<int> findClosestElements(vector<int>& arr, int k, int x)
    {
        ...
        while (res.size() < k)
        {
            ...
            if (left >= arr.begin() && right != arr.end())
            {
                ...
                if (closerTo(a, b))
                {
                    std::cerr << "compare a and b, push " << a << std::endl;
                    res.push_front(a);
                    --left;
                }
                else
                {
                    std::cerr << "compare a and b, push " << b << std::endl;
                    res.push_back(b);
                    ++right;
                }
            }
            ...
            {
                // std::cerr << "no rightside left -> " << *left << std::endl;
                res.push_front(*left);
                --left;
            }
            else if (right != arr.end()) // no left side
            {
                // std::cerr << "no leftside left -> " << *right << std::endl;
                res.push_back(*right);
                ++right;
            }
        }
