...
public:
    ...
    void _serialize(TreeNode* curr, std::ostringstream& oss)
    {
        curr ? oss << curr->val << "," : oss << "x,";
        if (!curr) return;
        _serialize(curr->left, oss);
        _serialize(curr->right, oss);
    }
    ...
    std::pair<TreeNode*, int> _deserialize(const std::vector<std::optional<int>>& data, int idx)
    {
        ...
        return {curr, rightIdx}; // return curr and end index of current subtree
    }
    ...
    TreeNode* deserialize(string data)
    {
        ...
        {
            if (token == "x") vals.push_back(std::nullopt);
