...
public:
    ...
    int get(int key)
    {
        ...
        incrementFreq(kData);
        std::cerr << "found " << kData.data->value << std::endl;
    ...
    void handleEviction()
    {
        if (mainMap_.size() >= capacity_)
        {
            ...
            std::cerr << "Min freq=" << minFreq << std::endl;
    ...
    void evict(int key)
    {
        std::cerr << "Evicting. key=" << key << std::endl;
