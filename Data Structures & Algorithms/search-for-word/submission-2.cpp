...
public:
    ...
    static constexpr char inUse = 'x';

    bool dfs(int row, int col, vector<vector<char>>& board, const string& word, int idx)
    {
        // failed search
        ...
        if (board.at(row).at(col) == inUse) return false;

        char temp = board.at(row).at(col);
        board.at(row).at(col) = inUse;
        ...
        if (temp == word.at(idx))
        {
            ...
            ++idx;
            if (
                dfs(row + 1, col, board, word, idx) ||
                dfs(row - 1, col, board, word, idx) ||
                dfs(row, col + 1, board, word, idx) ||
                dfs(row, col - 1, board, word, idx)
            ) return true;
        }
